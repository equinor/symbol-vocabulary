...
ex:ExampleBlock
    ...
    ## The following statement requires all instances of
    ## ex:ExampleBlock to have the symbol with the identifier
    ## ex:cl-symbol-123456789:
    sh:property [
        sh:path imf:symbol ;
        sh:hasValue ex:cl-symbol-111
    ] ;

    ## The following statement requires that the terminals of
...
ex:ExampleTerminal
    ...
    ## connectionPoint on the ex:cl-symbol-123456789:
    sh:property [
        sh:path ( imf:symbol sym:isPointOn ) ;
        sh:hasValue ex:cl-symbol-333
    ] ;
...
---------------------
example/example-type.ttl
rapper: Parsing URI file:///home/martige/repo/equinor/symbol-vocabulary/example/example-type.ttl with parser turtle
...
rapper: Parsing returned 262 triples
---------------------
#+end_example
...
#+RESULTS:
#+BEGIN_src ttl
Validating RDF data <example/example-type.ttl> against SHACL shape <http://shipshape.dyreriket.xyz/std-vocabulary-elements.ttl>:
@prefix dc:      <http://purl.org/dc/terms/> .
@prefix ex:      <http://example.com#> .
@prefix foaf:    <http://xmlns.com/foaf/0.1/> .
@prefix imf:     <http://ns.imfid.org/imf#> .
...
[ rdf:type     sh:ValidationReport ;
  ...
  sh:result    [ rdf:type                      sh:ValidationResult ;
                 ...
                 sh:resultMessage              "Unrecognised SKOS vocabulary element. Check spelling." ;
                 sh:resultSeverity             sh:Violation ;
                 sh:sourceConstraintComponent  sh:SPARQLConstraintComponent ;
                 sh:sourceShape                sss-sve:SKOSVocabularyShape ;
                 sh:value                      imf:symbol
               ]
] .

Validating RDF data <example/example-type.ttl> against SHACL shape <http://www.w3.org/ns/shacl-shacl#>:
@prefix rdf:  <http://www.w3.org/1999/02/22-rdf-syntax-ns#> .
@prefix rdfs: <http://www.w3.org/2000/01/rdf-schema#> .
@prefix sh:   <http://www.w3.org/ns/shacl#> .
@prefix xsd:  <http://www.w3.org/2001/XMLSchema#> .
